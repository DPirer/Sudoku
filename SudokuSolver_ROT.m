...
% SE ROTA LA IMAGEN, PARA ELLO SE CALCULA LA ORIENTACION
stats4=regionprops(BW4,'Orientation');
q=(stats4.Orientation)*(pi/180)/2;
BW3=imwarp(BW3,affine2d([cos(q) sin(q) 0;-sin(q) cos(q) 0;0 0 1]));
BW4=imwarp(BW4,affine2d([cos(q) sin(q) 0;-sin(q) cos(q) 0;0 0 1]));
[row,colom] = find(BW4==1);
BW4n = BW4(min(row):max(row),min(colom):max(colom));
BW3n=imcrop(BW3,[min(colom) min(row) (max(colom)-min(colom)) (max(row)-min(row))]);

%SE VUELVE A INTRODUCIR COMO REFERENCIA LA IMAGEN ROTADA
BW=BW3n;
BW2 = bwareafilt(imcomplement(BW),1,'largest');
[row,colom] = find(BW2==1);
BW3 = BW(min(row):max(row),min(colom):max(colom));
